...
const POSTS_URL = "https://d5vml3-8000.csb.app/post";
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(handleLike.fulfilled, (state, action) => {
        ...
        state.posts = state.posts.map((post) =>
          post._id === postId && !post.likes.includes(userId)
      ...
      .addCase(addComment.fulfilled, (state, action) => {
        ...
        state.posts = state.posts.map((post) =>
          post._id === postId
            ? { ...post, comments: [...post.comments, message] }
            : post,
        );
      })
